...
- **Client Communication**: WhatsApp chat module with tagging, assignment, order linking, voice message recording/sending (M4A/AAC format for iOS compatibility), chat renaming (admin/support), message deletion (admin/support), and full chat deletion with cascade (admin only). UI features include message truncation in chat list with professional icons (voice, video, photo, document), encryption notice banner, file/audio message display, voice message download button, message reactions on all messages with WhatsApp ID, chat pinning (unlimited, shown at top with pin icon), and message forwarding to multiple chats.
